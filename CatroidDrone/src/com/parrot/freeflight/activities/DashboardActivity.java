...
public class DashboardActivity extends DashboardActivityBase implements ServiceConnection, DroneAvailabilityDelegate,
		NetworkChangeReceiverDelegate, DroneFirmwareCheckReceiverDelegate, DroneConnectionChangeReceiverDelegate,
		MediaReadyDelegate {
	...
	public void onDroneAvailabilityChanged(boolean droneOnNetwork) {
		if (droneOnNetwork) {
			...
			if (droneOnNetwork) {
				// If we connected to the drone we are 100% sure that there is
	...
	private void onNotifyAboutGPSDisabled() {
		showAlertDialog(getString(R.string.Location_services_alert),
				getString(R.string.If_you_want_to_store_your_location_anc_access_your_media_enable_it), null);
	}
	...
	private void onNotifyAboutUSBStickRemove() {
		...
		showAlertDialog(getString(R.string.Update_Warning),
				getString(R.string.Please_unplug_your_USB_key_from_the_ARDrone_now), actionOnDismiss);
	}
