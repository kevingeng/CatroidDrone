...
public class GLBGVideoSprite extends GLSprite {
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		// # on GALAXY S3 MINI
		this.screenWidth = width; // # 480 : for window size changing do it on
									// this way: this.screenWidth = width - diff
		this.screenHeight = height; // # 800 : you don't have to change it
			...
			if (getVideoFrameNative(video, videoSize)) {
				...
				if (newVideoWidth != videoWidth || newVideoHeight != videoHeight) {
					...
					if (videoWidth != 0 && videoHeight != 0) {
						matrix.setScale((float) screenHeight / (float) videoHeight, (float) screenHeight
								/ (float) videoHeight);
					}
				}
